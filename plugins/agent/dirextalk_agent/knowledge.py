...
class KnowledgeStore:
    ...
    def _init_db(self) -> None:
        with self._connect() as conn:
            conn.executescript(
                ...
                CREATE UNIQUE INDEX IF NOT EXISTS knowledge_sources_sha_idx
            ...
            conn.execute(
                "INSERT OR IGNORE INTO knowledge_config (id, enabled, updated_at) VALUES (1, 0, ?)",
                (int(time.time()),),
            )
    ...
    def config_get(self) -> dict[str, Any]:
        self._touch()
        with self._connect() as conn:
            row = conn.execute("SELECT enabled, updated_at FROM knowledge_config WHERE id = 1").fetchone()
        return {
    ...
    def config_update(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            conn.execute(
                "UPDATE knowledge_config SET enabled = ?, updated_at = ? WHERE id = 1",
    ...
    def source_delete(self, source_id: str) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            row = conn.execute("SELECT sha256 FROM knowledge_sources WHERE source_id = ?", (source_id,)).fetchone()
            deleted = conn.execute("DELETE FROM knowledge_sources WHERE source_id = ?", (source_id,)).rowcount
        if row:
    ...
    async def upload_finish(
        ...
        *,
        ...
        with self._connect() as conn:
            existing = conn.execute("SELECT source_id, status FROM knowledge_sources WHERE sha256 = ?", (sha,)).fetchone()
        if existing:
    ...
    async def memory_create(
        ...
        *,
        ...
        with self._connect() as conn:
            existing = conn.execute("SELECT source_id FROM knowledge_sources WHERE sha256 = ?", (sha,)).fetchone()
        if existing:
            return {"ok": True, "deduplicated": True, "source": self._source_by_id(str(existing["source_id"]))}
        async with self._index_lock:
            source = await self._index_text(
    ...
    def status(self) -> dict[str, Any]:
        self._touch()
        with self._connect() as conn:
            source_count = int(first_value(conn.execute("SELECT COUNT(*) FROM knowledge_sources").fetchone()) or 0)
            chunk_count = int(first_value(conn.execute("SELECT COUNT(*) FROM knowledge_chunks").fetchone()) or 0)
            total_bytes = int(
                first_value(conn.execute("SELECT COALESCE(SUM(size), 0) FROM knowledge_sources").fetchone()) or 0
            )
        return {
            "ok": True,
        ...
        *,
        ...
        with self._connect() as conn:
            ...
            for position, (chunk, embedding) in enumerate(zip(chunks, embeddings, strict=True)):
                ...
                conn.execute(
                    """
                    INSERT INTO knowledge_chunks (chunk_id, source_id, position, text, embedding_json)
                    VALUES (?, ?, ?, ?, ?)
                    """,
                    (
                        chunk_id,
    ...
    def _check_capacity(self, incoming_bytes: int) -> None:
        with self._connect() as conn:
            total = int(first_value(conn.execute("SELECT COALESCE(SUM(size), 0) FROM knowledge_sources").fetchone()) or 0)
    ...
    def _check_chunk_capacity(self, incoming_chunks: int) -> None:
        with self._connect() as conn:
            total = int(first_value(conn.execute("SELECT COUNT(*) FROM knowledge_chunks").fetchone()) or 0)
